...
{
    QString fileName = QFileDialog::getSaveFileName(this, tr("Сохранить рисунок"), QString(), tr("PaintProj (*.image)"));
    if(fileName.isEmpty()) return;
    if(!paintArea->saveFile(fileName)) QMessageBox::warning(this, tr("Сохранение не удалось"), tr("Could not save file"));
}

void MainWindow::onActionLoad()
{
    QString fileName = QFileDialog::getOpenFileName(this, tr("Открыть рисунок"), QString(), tr("PaintProj (*.image)"));
    if(fileName.isEmpty()) return;
    if(!paintArea->loadFile(fileName)) QMessageBox::warning(this, tr("Не удалось открыть"), tr("Could not load file"));
}
